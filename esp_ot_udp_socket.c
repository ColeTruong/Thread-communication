...
{
    if (udp_client_member == NULL || udp_client_member->sock < 0)
    {
        ESP_LOGE(OT_EXT_CLI_TAG, "Invalid UDP client or socket not initialized");
        return;
    }

    struct sockaddr_in dest_addr4 = {0};  // For IPv4
    struct sockaddr_in6 dest_addr6 = {0}; // For IPv6
    int len = 0;
    int family = AF_INET6;  // Default to IPv6

    // Check if the destination IP address is IPv4 or IPv6
    if (inet_pton(AF_INET, udp_client_member->dest_ipaddr, &dest_addr4.sin_addr) == 1)
    {
        // It's an IPv4 address
        family = AF_INET;
        dest_addr4.sin_family = AF_INET;
        dest_addr4.sin_port = htons(udp_client_member->dest_port);
    }
    else if (inet_pton(AF_INET6, udp_client_member->dest_ipaddr, &dest_addr6.sin6_addr) == 1)
    {
        // It's an IPv6 address
        family = AF_INET6;
        dest_addr6.sin6_family = AF_INET6;
        dest_addr6.sin6_port = htons(udp_client_member->dest_port);
    }
    else
    {
        ESP_LOGE(OT_EXT_CLI_TAG, "Invalid destination IP address: %s", udp_client_member->dest_ipaddr);
        return;
    }

    // Log the address and port
    ESP_LOGI(OT_EXT_CLI_TAG, "Sending to %s : %d", udp_client_member->dest_ipaddr, udp_client_member->dest_port);

    // Bind the socket to the network interface (if necessary)
    //esp_err_t err = socket_bind_interface(udp_client_member->sock, &(udp_client_member->ifr));
    //if (err != ESP_OK)
    //{
      //  ESP_LOGE(OT_EXT_CLI_TAG, "Failed to bind socket to interface: %s", esp_err_to_name(err));
        //return; // Stop sending message if bind fails
    //}

    // Send the message over UDP based on address family
    if (family == AF_INET)
    {
        // Send over IPv4
        len = sendto(udp_client_member->sock, udp_client_member->messagesend.message,
                     strlen(udp_client_member->messagesend.message), 0,
    ...
    if (len < 0) 
    {
        ESP_LOGW(OT_EXT_CLI_TAG, "Failed to send message, error: %d", errno);
    }
    else
    {
        ESP_LOGI(OT_EXT_CLI_TAG, "Message sent: %s", udp_client_member->messagesend.message);
    }
}
